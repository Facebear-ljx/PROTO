...
def update_q_online(critic: Model, target_critic: Model, target_online_actor: Model, online_actor: Model, behavior: Model, offline_actor: Model, batch: Batch,
    ...
    rng1, rng2, rng3, rng4, rng5, rng = jax.random.split(key, num=6)
    ...
    next_acts = dist.sample(seed=rng3)

    if args.sac and args.entropy_backup:
        value_reg = log_alpha() * dist.log_prob(next_acts) + temp_online * (dist.log_prob(next_acts) - dist_k.log_prob(next_acts))
    else:
        value_reg = 0.
        
    if double:
        target_qs = target_critic(batch.next_observations, next_acts, rngs={"dropout": rng4})
    else:
